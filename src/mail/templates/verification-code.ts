/**
 * Template HTML do e-mail de verificação de cadastro.
 * Identidade visual alinhada às telas do InvestWealth.
 */
export function verificationCodeTemplate(name: string, code: string): string {
  return `<!doctype html>
<html lang="pt-BR">
      ...
      <tr><td align="center">
        <table width="480" cellpadding="0" cellspacing="0" style="background:#111827;border:1px solid #1f2937;border-radius:12px;overflow:hidden;">
          <tr><td style="padding:24px 32px;border-bottom:1px solid #1f2937;">
            ...
            <p style="margin:4px 0 0;font-size:11px;text-transform:uppercase;letter-spacing:0.18em;color:#64748b;">Verificação de Cadastro</p>
          </td></tr>
          <tr><td style="padding:32px;">
            <h1 style="margin:0 0 12px;font-size:22px;color:#f8fafc;">Protocolo Iniciado</h1>
            <p style="margin:0 0 20px;font-size:14px;line-height:1.6;color:#cbd5e1;">
              Olá, <strong style="color:#f8fafc;">${escapeHtml(name)}</strong>. Confirme o seu endereço de e-mail institucional
              para ativar a sua conta na plataforma.
            </p>
            <div style="text-align:center;padding:24px;background:#0b1220;border:1px solid #1f2937;border-radius:8px;">
              <p style="margin:0 0 8px;font-size:11px;text-transform:uppercase;letter-spacing:0.18em;color:#64748b;">Código de Verificação</p>
            ...
            <p style="margin:20px 0 0;font-size:12px;color:#64748b;">O código expira em 15 minutos. Se você não solicitou este cadastro, ignore este e-mail.</p>
          </td></tr>
...
export function verificationCodeSubject(): string {
  return 'InvestWealth — Código de Verificação de Cadastro';
}
